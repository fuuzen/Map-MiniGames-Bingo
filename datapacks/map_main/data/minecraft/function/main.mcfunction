...
execute as @a[scores={leave=1..}] in bingo:lobby run tp @s 188 124 26
...
tag @a[gamemode=survival,nbt={Dimension:"bingo:lobby"}] add NEWENTER
execute in bingo:lobby run tp @a[tag=NEWENTER] 188 124 26 0 0
...
execute if score battle.state state matches 1.. in bingo:lobby run function battle/tick
execute if score btw.state state matches 1.. in bingo:lobby run function boatwars/tick
...
execute as @a[tag=join.snow] in bingo:lobby at @s run function snow/join
tag @a[tag=join.snow] remove join.snow

effect give @a[team=wait.total] resistance 2 25 true
execute as @a[tag=join.livelongest] in bingo:lobby run tp @s 25 7 0 0 0
...
execute in bingo:lobby as @a[x=137,y=89,z=-158,dx=2,dy=2,dz=2,gamemode=adventure] at @s run function hide2/died
